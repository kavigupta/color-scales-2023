...
plt.title("2023 Proposal [same selection as 2022]")
plot_ramps(proposal_2022_normalized.hsv)
plt.savefig("proposal_ramps/2023_selected_as_2022.png")
plt.close()

plt.figure(figsize=(3, 50))
plt.title("2023 Proposal [all]")
plot_ramps(
    proposal_2023_for_hues(
        np.array([x for y in range(10) for x in range(360) if x % 10 == y])
    ).hsv
)
plt.savefig("proposal_ramps/2023_all.png")
plt.close()


plt.title("2023 Proposal [selected]")
plot_ramps(
    proposal_2023_for_hues(
        np.array([2, 32, 48, 134, 180, 210, 220, 230, 240, 250, 280, 300, 330])
    ).hsv
)
plt.savefig("proposal_ramps/2023_selected.png")
plt.close()
